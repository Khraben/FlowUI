import React from 'react';

export const FlagEN = ({ className = 'w-6 h-6' }: { className?: string }) => (
  <svg className={className} viewBox="0 0 496 496" xmlns="http://www.w3.org/2000/svg">
    <path
      fill="#29337A"
      d="M496,369.6c0,27.2-21.6,46.4-48,46.4H48c-26.4,0-48-19.2-48-46.4V126.4C0,99.2,21.6,80,48,80h400c26.4,0,48,19.2,48,46.4V369.6z"
    />
    <path fill="#1B265B" d="M48,80h400c26.4,0,48,19.2,48,46.4v243.2c0,27.2-25.6,46.4-52,46.4" />
    <path fill="#111D49" d="M48,80h400c26.4,0,48,19.2,48,46.4V216" />
    <path fill="#111D49" d="M496,368.8c0,27.2-21.6,47.2-48,47.2H48c-26.4,0-48-20.8-48-48" />
    <polygon
      fill="#FFFFFF"
      points="272,248 442.4,80 394.4,80 248,226.4 101.6,80 53.6,80 224,248 53.6,416 101.6,416 248,269.6 394.4,416 442.4,416"
    />
    <polygon fill="#D9ECED" points="102.4,368 53.6,416 101.6,416 149.6,368" />
    <polygon fill="#D9ECED" points="346.4,368 394.4,416 442.4,416 393.6,368" />
    <polygon
      fill="#D9ECED"
      points="411.2,386.4 272,249.6 442.4,80 394.4,80 248,226.4 101.6,80 53.6,80 85.6,110.4"
    />
    <polygon
      fill="#FFFFFF"
      points="496,216 280,216 280,80 216,80 216,216 0,216 0,280 216,280 216,416 280,416 280,280 496,280"
    />
    <rect x="216" y="368" fill="#D9ECED" width="64" height="48" />
    <polygon
      fill="#D9ECED"
      points="280,80 216,80 216,216 210.4,216 285.6,280 496,280 496,216 280,216"
    />
    <polygon
      fill="#E51D35"
      points="496,232 264,232 273.6,232 432.8,80 416.8,80 264,232 264,80 232,80 232,232 232,232 77.6,80 63.2,80 216,232 0,232 0,264 216.8,264 67.2,416 81.6,416 232,264 232,416 264,416 264,265.6 418.4,416 432.8,416 280,264 496,264"
    />
    <rect x="232" y="368" fill="#AF0026" width="32" height="48" />
    <polygon fill="#AF0026" points="114.4,368 67.2,416 81.6,416 129.6,368" />
    <polygon fill="#AF0026" points="369.6,368 418.4,416 432.8,416 384,368" />
    <polygon
      fill="#AF0026"
      points="278.4,232 432.8,80 416.8,80 264,232 264,80 232,80 232,232 232,232 77.6,80 63.2,80 151.2,167.2 343.2,328.8 280,264 496,264 496,232 264,232"
    />
  </svg>
);
